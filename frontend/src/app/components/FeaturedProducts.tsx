...
export const ProductCard = ({ product, onAddToCart }: { product: any, onAddToCart: (p: any) => void }) => {
  return (
    <div className="bg-white/10 backdrop-blur-md rounded-2xl shadow-xl border border-white/20 overflow-hidden group hover:border-white/40 transition-all duration-300">
      ...
      <div className="p-5">
        <div className="flex items-center gap-1 text-cyan-400 mb-2">
          ...
          <span className="text-xs font-bold text-cyan-200/60">{product.rating}</span>
        </div>
        <h3 className="font-bold text-white mb-1 line-clamp-1 group-hover:text-cyan-300 transition-colors">{product.name}</h3>
        <p className="text-xs text-blue-100/50 mb-4 line-clamp-2 leading-relaxed">{product.description}</p>
        <div className="flex items-center justify-between mt-auto">
          <div className="flex flex-col">
            {product.isOnSale && (
              <span className="text-[10px] text-white/30 line-through">€{product.originalPrice?.toFixed(2)}</span>
            )}
            <span className="text-xl font-black text-cyan-300">€{product.price.toFixed(2)}</span>
          </div>
          <button 
...
export const FeaturedProducts = ({ onAddToCart }: { onAddToCart: (p: any) => void }) => {
    ...
    <section className="py-24 relative z-10">
      <div className="container mx-auto px-4">
        <div className="flex items-end justify-between mb-12">
          <div>
            <h2 className="text-3xl font-black text-white mb-2">Prodotti in Evidenza</h2>
            <p className="text-blue-100/60 font-medium">Le nostre migliori scelte per il tuo acquario.</p>
          </div>
          <button className="text-cyan-400 font-bold hover:text-cyan-300 transition-colors flex items-center gap-1 group">
            Vedi Tutti <span className="group-hover:translate-x-1 transition-transform">→</span>
          </button>
        </div>
